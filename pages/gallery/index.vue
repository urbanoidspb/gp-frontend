    ...
    <section class="gallery">
      ...
      <main>
        <div class="main_item" v-for="gallery in galleries.slice(0, this.galleriesOnPage)" v-bind:key="gallery.id">
          ...
          <Gallery v-bind:gallery-id="gallery.id" v-bind:gallery-photos="gallery.photos"
                   v-bind:gallery-time="gallery.time"/>
          <h2 class="name">{{funcTitle(gallery.title)}}</h2>
        </div>
      </main>
      <div class="showallRow">
        <button v-on:click="galleriesOnPage += 6"
                v-if="!(this.galleriesOnPage >= galleries.length)" class="showall"
                to="/about">Показать ещё
        </button>
      </div>
    </section>
  </div>
